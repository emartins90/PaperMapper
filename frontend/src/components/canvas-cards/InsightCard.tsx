...
export default function InsightCard({ data, showHandles = true, width = 'w-96', openCard, showArrow = true, showShadow = true, showIcon = true }: InsightCardProps) {
  ...
  return (
    <div 
      className={`rounded-xl border-2 bg-white ${showCondensed ? 'p-2.75' : 'p-4'} ${effectiveWidth} relative transition-all duration-200 cursor-pointer
        ...
        ${data.isSelected 
          ? `border-insight-400 ${effectiveShadow ? 'shadow-lg shadow-insight-200/50' : ''}` 
          : `border-insight-300 hover:border-insight-400 ${effectiveShadow ? 'hover:shadow-lg' : ''}`
        }`}
    ...
    >
      ...
      <div className={`flex items-center justify-between ${showCondensed ? 'mb-0!' : 'mb-2'}`}>
        <div className="text-insight-700 truncate-block flex items-center gap-1">
          {showIcon && <LuLightbulb className="text-insight-400" size={22} />}
          ...
          <button onClick={data.onOpen} aria-label="Open card" className="cursor-pointer">
            <span className={`text-xl transition-all duration-200 inline-block ${
              openCard && data.cardId && openCard.id === data.cardId 
                ? `text-insight-600 rotate-45 ${data.panelJustOpened ? '' : 'hover:text-insight-400 hover:rotate-0'}` 
                : `text-insight-400 ${data.panelJustOpened ? '' : 'hover:text-insight-600 hover:rotate-45'}`
            }`}>↗</span>
          </button>
        )}
      </div>
