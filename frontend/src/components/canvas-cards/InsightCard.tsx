...
export default function InsightCard({ data, showHandles = true, width = 'w-96', openCard, showArrow = true, showShadow = true }: InsightCardProps) {
  ...
  return (
    <div 
      className={`rounded-xl border-2 bg-white p-4 ${width} relative transition-all duration-200 cursor-pointer
        ...
        ${data.isSelected 
          ? `border-insight-400 ${showShadow ? 'shadow-lg shadow-insight-200/50' : ''}` 
          : `border-insight-300 hover:border-insight-400 ${showShadow ? 'hover:shadow-lg' : ''}`
        }`}
