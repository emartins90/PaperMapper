...
export default function ThoughtCard({ data, showHandles = true, width = 'w-96', openCard, showArrow = true, showShadow = true, showIcon = true }: ThoughtCardProps) {
  ...
  return (
    <div 
      className={`rounded-xl border-2 bg-white ${showCondensed ? 'p-2.75' : 'p-4'} ${effectiveWidth} relative transition-all duration-200 cursor-pointer
        ...
        ${data.isSelected 
          ? `border-thought-400 ${effectiveShadow ? 'shadow-lg shadow-thought-200/50' : ''}` 
          : `border-thought-300 hover:border-thought-400 ${effectiveShadow ? 'hover:shadow-lg' : ''}`
        }`}
    ...
    >
      ...
      <div className={`flex items-center justify-between ${showCondensed ? 'mb-0!' : 'mb-2'}`}>
        <div className="font-bold text-thought-700 flex items-center gap-1">
          {showIcon && <LuMessageCircle className="text-thought-400" size={22} />}
          Thought
        </div>
        {showArrow && (
      ...
      {!showCondensed && data.files && data.files.length > 0 && (
        ...
        {showCondensed && data.files && data.files.length > 0 && (
        (() => {
          const hasMainText = data.thought && data.thought.trim() !== '';
